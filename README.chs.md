...
**使用场景**:
- **高精度**: 10-30秒（更频繁同步，更高精度）
- **标准**: 60秒（平衡，推荐大多数情况）
- **资源受限**: 120-300秒（降低 NTP 服务器负载）
...
**使用场景**:
- **严格同步**: 漂移 20-30ms, 间隔 5-10秒
- **标准**: 漂移 50ms, 间隔 10-60秒（默认）
- **宽松**: 漂移 100ms, 间隔 120秒+（节省流量）
...
- ⚙️ **自定义接收器同步间隔**: 为接收器添加 `NTP Sync Interval` 设置
  - 用户现在可以自定义 NTP 同步之间的最小时间间隔
  - 范围: 100ms 到 1小时（默认值: 10秒）
  - 替代了硬编码的 10秒限制，将控制权完全交给用户
  - **警告**: 设置过低（如 < 1000ms）可能在慢速网络下导致卡顿
  - 添加了上下文 UI 警告以引导用户

### v1.1.2 (2026-01-04)

**🔧 Bug 修复:**
- 🐛 **修复接收器卡顿**: NTP 同步的关键性能修复
  - 为关键帧触发的 NTP 同步添加最小10秒间隔
  - 防止过多的网络请求（之前每2秒触发一次）
  - 解决视频播放卡顿问题
...
**技术细节:**
- 接收器现强制执行 NTP 同步间的10秒最小间隔
- 关键帧同步: `is_keyframe && (距离上次同步 >= 10秒)`
- 漂移检测保持不变（用户可配置阈值）
