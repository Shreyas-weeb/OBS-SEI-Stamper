...
📺 **[YouTubeでデモ動画を見る](https://youtu.be/9aJCHxzy-ME)** *(中国語)*

このデモンストレーションでは、OBSから同じ設定で4つのSRTストリームをローカルネットワーク経由で送信しています。ネイティブのOBSメディアソースではストリーム間の同期ができませんが、本プラグインを使用すると、4つのストリーム全てが±1フレーム以内の精度で同期されます。
